...
- **A scheduled job on a personal machine needs to survive the machine being off at trigger time.** systemd timers get this via `Persistent=true` (catches up next activation, doesn't wake the machine); `schtasks.exe` has no equivalent CLI flag, so a task registered with it silently never runs on a day the trigger was missed. Fix: register with `schtasks` as before (proven, don't touch it), then a follow-up `Set-ScheduledTask` step sets just `StartWhenAvailable` on the already-registered task — smaller, lower-risk diff than rewriting registration in PowerShell from scratch, and if the follow-up step fails the task is still installed and functional, just without catch-up.
- **Windows-only code (`#[cfg(target_os = "windows")]`) can't be compiled from Linux without a cross toolchain** (`ring`/rustls needs a C compiler for the target — `x86_64-w64-mingw32-gcc`). Full `mingw-w64` is a heavy, multi-variant install; not worth it on a disk-constrained box for occasional cross-checks. CI's `windows-latest` runner is the real compile check — write Windows-only code carefully by hand, keep diffs small and additive where behavior can't be locally verified, and say so explicitly rather than implying it's been tested.
